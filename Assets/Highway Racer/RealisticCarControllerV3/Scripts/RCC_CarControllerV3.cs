...
public class RCC_CarControllerV3 : MonoBehaviour
{
    ...
    internal float _brakeInput
    {
        get
        {
            ...
            if (!automaticGear || semiAutomaticGear)
            {
                if (!SceneManager.GetActiveScene().name.ToLower().Contains("multi"))
                {
    ...
    internal float _handbrakeInput
    {
        get
        {
            if (!SceneManager.GetActiveScene().name.ToLower().Contains("multi"))
    ...
    HR_OptionsHandler hroptionshandler;
    void Awake()
    {
        instance = this;
        if (!SceneManager.GetActiveScene().name.ToLower().Contains("multi"))
        {
            hroptionshandler = FindObjectOfType<HR_OptionsHandler>();
        }
    ...
    private void Fuel()
    {
        ...
        if (!SceneManager.GetActiveScene().name.ToLower().Contains("multi"))
        {
            if (hroptionshandler)
            {
                hroptionshandler.fuelSlider.value = fuelTank;
                //  print("fule");
            }
            if (hroptionshandler)
            {
                if (hroptionshandler.fuelSlider.value < 15)
                {
                    hroptionshandler.lowFueltext.SetActive(true);
                    HR_OptionsHandler.instance.alarm.enabled = true;
                }
                if (hroptionshandler.fuelSlider.value < 2)
                {
                    hroptionshandler.lowFueltext.SetActive(false);
                }
        ...
        //.fuelSlider.value = fuelTank;
        //if (HR_OptionsHandler.instance.fuelSlider.value < 15)
        //{
        //    HR_OptionsHandler.instance.lowFueltext.SetActive(true);
        //}
    }
